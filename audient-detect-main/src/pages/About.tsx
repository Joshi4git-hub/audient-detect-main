...
const About = () => {
  ...
  // Listen for voice navigation commands and (optionally) add more interaction
  const { isListening, isSpeaking, speak, startListening } = useVoiceRecognition({
    onResult: (transcript) => {
      ...
      if (text.includes("go to home") || text === "home") {
        speak("Going to home").then(() => navigate("/Home"));
        return;
      }
  ...
  useEffect(() => {
    setTimeout(() => {
      speak(
        "About Object Detection Application: An AI-powered interactive application that detects and narrates objects in your videos using natural voice communication."
      ).then(() => startListening());
    }, 1000);
  }, []);
